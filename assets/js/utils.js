...
window.showToast = function ({ title, message, type, duration = 5000, text_center = true }) {
...
// window.loadPageContent = function ({ html_content, container_id }) {
//     $(`#${container_id}`).html(html_content)
// }
...
window.showErrorToast = function ({ response, title = "Something went wrong!", message = "", type = "error", duration = 5000, text_center = true } = {}) {
    const status = response?.status ?? -1; // fallback to -1 if undefined
    if (status == 401) handleSessionExpired(duration);
    else if (status === 0) showToast({ title: "Server Unreachable", message: "Could not connect to the server. Please try again later.", type: "error", duration, text_center });
...
window.setupModalWithOutsideClose = function (modalId, closeBtnId, alertSelector = null, func) {
