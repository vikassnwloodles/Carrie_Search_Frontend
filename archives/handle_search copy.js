...
window.bindSearchHandler = function () {
    ...
    window.renderSearchResults = function (data, loadingHtmlContainerId = null) {
        console.log(data.relatedQuestions)
        console.log("BEFORE ERROR.........")
        const uniqueId = Date.now() + Math.floor(Math.random() * 1000); // ensure uniqueness
        const withLineBreaks = data.query.replace(/\n/g, "<br>");
                                console.log("BEFORE ERROR.........")
        let resultsHtml = `
        <div id="individual-search-result-${uniqueId}">
        <div class="animate-fade-in text-left mb-8 p-6 bg-white rounded-lg border border-gray-200 relative">
                        ...
                        <span>DOCX</span>
                                ...
                                <div class="relative group/edit">
                                    <div
                                        ...
                                        onclick="handleRewrite('${uniqueId}')"
                                        class="flex items-center justify-center p-2 bg-white h-8 gap-1 rounded-md text-gray-600  hover:text-teal-600 transition-colors cursor-pointer">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7999999999999998" stroke-linecap="round" stroke-linejoin="round" class="tabler-icon tabler-icon-repeat "><path d="M4 12v-3a3 3 0 0 1 3 -3h13m-3 -3l3 3l-3 3"></path><path d="M20 12v3a3 3 0 0 1 -3 3h-13m3 3l-3 -3l3 -3"></path></svg>                                        
    ...
    <div class="relative group">
        ...
        <div id="result-copy-tooltip-${uniqueId}"
            ...
            Copy to clipboard
                ...
                <div id="response-text-inner-${uniqueId}">${data.content}</div>
            </div>
            </div>
            ${renderRelatedQuestionsContainer(data.relatedQuestions, uniqueId)}
            </div>
            <script>
                $('#copy-icon-${uniqueId}').on('click', function () {
                    ...
                    const textHtml = $('#response-text-inner-${uniqueId}')
                        .html()
                ...
                function handleRewrite(localUniqueId){
                    $("#rewrite-btn-"+localUniqueId).addClass("pointer-events-none")
                    $("#response-text-inner-"+localUniqueId).html("Rewriting...");
                    const queryText = $("#query-text-"+localUniqueId).html()
                        .replace(/<([a-z]+)[^>]*>(?:\\s|&nbsp;|\\u200B)*<\\/\\1>/gi, '')
                        .replace(/<br\\s*\\/?>/gi, '\\n')
                        .replace(/<\\/p>/gi, '\\n\\n')
                        .replace(/<\\/li>/gi, '\\n')
                    ...
                    call_search_api(queryText)
                    .then(response => {
                        searchContent = response.choices[0].message.content
                        content = structuredData(searchContent);
                        content = content.replace(/<table([\\s\\S]*?)<\\/table>/g, function (match) {
                            return \`<div class="overflow-x-auto w-full my-4 rounded-md border border-gray-200">match</div>\`;
                        });
                        $("#response-text-inner-"+localUniqueId).html(content);
                    })
                    .catch(error => {
                        showToast({ response: error })
                    })
                    .always(() => {
                        $("#rewrite-btn-"+localUniqueId).removeClass("pointer-events-none")
                    });
                }
